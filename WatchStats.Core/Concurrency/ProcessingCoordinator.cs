...
namespace WatchStats.Core.Concurrency
{
    ...
    public sealed class ProcessingCoordinator
    {
        ...
        {
            public static readonly EventId CoordinatorWorkerStarted = new(6, "coordinator_worker_started");
            public static readonly EventId CoordinatorWorkerStopped = new(7, "coordinator_worker_stopped");
        }
        ...
        public ProcessingCoordinator(BoundedEventBus<FsEvent> bus, FileStateRegistry registry, IFileProcessor processor,
            WorkerStats[] workerStats, int workerCount = 4, int dequeueTimeoutMs = 200, ILogger<ProcessingCoordinator>? logger = null)
        {
        ...
        private void WorkerLoop(int workerIndex)
        {
            ...
            _logger?.LogInformation(Events.CoordinatorWorkerStarted,
                "Worker thread started. WorkerId={WorkerId} ThreadId={ThreadId}",
            ...
            {
                _logger?.LogInformation(Events.CoordinatorWorkerStopped,
                    "Worker thread stopped. WorkerId={WorkerId}",
                    workerIndex);
            }
        }
